...
--Overview of SubCategory 'Other' in 'Purchases &Payments' and (it accounts around 32% of the total transactions amount)
Select Date, Description, Amount
FROM BankStatement
WHere TransactionType = 'Debit' AND Category = 'Purchases & Payments' AND SubCategory = 'Other'
...
SELECT 
    FORMAT(Date, 'yyyy-MM') AS Month,
    AVG(Amount) AS AverageGroceriesSpent
FROM BankStatement
...
SELECT Date, Description, Amount
FROM BankStatement
WHERE Category = 'Banking Fees' AND FORMAT(Date, 'yyyy-MM') = '2024-10'

SELECT Description, SUM(Amount) AS TotalFees 
FROM BankStatement
WHERE Category = 'Banking Fees'
...
SELECT FORMAT(Date, 'yyyy-MM') AS Month, Description, SUM(Amount) AS TotalFees 
FROM BankStatement
WHERE Category = 'Banking Fees'
GROUP BY Description, FORMAT(Date, 'yyyy-MM')
